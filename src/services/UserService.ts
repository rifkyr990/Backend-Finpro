...
class UserService {
    public async updateProfilePicture(userId: string, fileBuffer: Buffer) {
        ...
        if (user?.image_id) { await cloudinary.uploader.destroy(user.image_id);}
    ...
    private buildProfileUpdateData(data: any, existingEmail?: string) {
        ...
        if (email && email !== existingEmail) { updateData.email = email; updateData.is_verified = false; }
        return updateData;
    }

    public async updateProfile(userId: string, data: any) {
        if (!data) throw new Error("Request body kosong, pastikan mengirim data profile");
    ...
    public async verifyNewEmail(token: string) {
        ...
        if (isInvalid) throw new Error("Token tidak valid atau sudah kadaluarsa");
        if (!tokenRecord.user) throw new Error("User tidak ditemukan");
    ...
    private async markTokenAsUsed(tokenId: number) {

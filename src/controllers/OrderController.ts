  ...
  public static adminCancelOrder = asyncHandler(
    async (req: AuthRequest, res: Response) => {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        // Send a confirmation email to the user automatically
  ...
  public static markAsRefunded = asyncHandler(
    async (req: AuthRequest, res: Response) => {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const paymentToRefund = order.payments.find(p => p.status === 'SUCCESS');
        if (!paymentToRefund) {
             throw new Error("Could not find a successful payment record to refund.");
        }
